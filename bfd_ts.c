...
bfd_ts_configure(struct bfd *bfd, const char *name, const struct smap *cfg)
{
    ...
    setting.forwarding_override= smap_get_bool(cfg, "forward_override", false);
                  ...
                  uint8_t eth_src[ETH_ADDR_LEN],  long long int now)
{
    ...
    memcpy(eth->eth_dst, eth_addr_bfd, ETH_ADDR_LEN);
...
{
    bool ret;
    ret = bfd_should_process_packet(flow->dl_type, flow->nw_proto,
                                    flow->tp_dst);
